...
type SearchOp = DataOp<'/data/v1/news/search', 'post'>;
type EnrichOp = DataOp<'/data/v1/news/enrich', 'post'>;

export class News extends Resource {
  search(body: BodyOf<SearchOp>): Promise<Page<unknown>> {
    return this.postPage<unknown>('/data/v1/news/search', body);
  }
  ...
  enrich(body: BodyOf<EnrichOp>): Promise<ResponseOf<EnrichOp>> {
    return this.post<ResponseOf<EnrichOp>>('/data/v1/news/enrich', body);
  }
}
